...
> 版本：v1.0 | 日期：2026-03-09 | 周期：6 周 | 团队：2 人
...
- [ ] `pnpm test`（合约）全部通过，覆盖率 ≥ 85%
- [ ] MTToken 和 CourseManager 成功部署到 Sepolia，可在 Etherscan 查看
- [ ] 前端 MetaMask 连接/断开功能正常
- [ ] 非 Sepolia 网络时显示切换提示
- [ ] MT 余额可正确从链上读取显示
...
- [ ] `GET /auth/nonce` + `POST /auth/verify` 联调通过
- [ ] JWT Cookie 正确设置，`/users/me` 可返回用户信息
- [ ] 课程发布完整流程：填表 → 上传 R2 → 链上存证 → 写 Supabase
- [ ] 课程广场可展示真实课程数据
- [ ] 搜索/排序功能可用
...
- [ ] 完整购买链路：余额检查 → approve → purchaseCourse → 链上确认 → 解锁
- [ ] TxProgress 4 种状态展示正确
- [ ] 个人中心可展示已购课程和发布课程
- [ ] 课程学习页权限控制（未购买跳转详情页）
- [ ] 预签名 URL 15 分钟后失效，14 分钟静默刷新
...
- [ ] AAVE supply 完整流程：approve USDT → supply → aToken 余额更新
- [ ] AAVE withdraw 完整流程：withdraw → 余额减少
- [ ] 收益总览数据准确
- [ ] 收益趋势图可展示历史数据
- [ ] 质押记录可查询，txHash 可跳转 Etherscan
